...
- 支持内容哈希和增量更新；
- 支持标签映射、重复检测和导入审计；
- 支持多份已人工核对的讲座提取稿整批发布；同批草稿必须一次校验、一次合并、一次重建搜索索引，并共同成功或共同退回待审核状态；
- 原始层、清理层、分析层和索引层分离；
- 索引可以重建，原始数据不可因重建丢失；
- 导入失败必须记录文件、原因和重试状态。
...
- `case_id`
- `lecture_id`
- `source_date`（由案例绑定的来源讲座当前发布版本确定性派生，用于年份筛选和排序）
- `case_type`
- `academic_label`
- `background`
- `curriculum_system`
...
阶段3在已有中文关键词金标技术评测通过后，仅为`lectures`和`cases`启用固定版本的中文嵌入器；`transcript_segments`继续受逐字稿隐私门禁，不自动生成向量。向量只保存在可重建的Meilisearch索引中，PostgreSQL中的知识版本和不可变来源对象仍是真相源。

智能搜索采用关键词与向量混合召回，不使用纯向量结果直接生成结论。语义权重必须通过版本化智能搜索金标比较后确定；当前技术草案比较`0.25`、`0.5`、`0.75`后采用`0.5`，仍需项目负责人完成业务批准。索引重建或嵌入失败时保留旧索引并回退普通关键词搜索。
...
- 由受限Agent把自然语言问题转换为最多六个结构化检索方案；
- 使用关键词+语义混合召回，合并候选后再由模型基于上下文过滤和重排；
- 模型只能引用实际召回的冻结知识版本，不能生成新来源、直接查询数据库或扩大搜索数据域；
- 查询规划最多两轮，设置候选、上下文、Token、超时和重试上限；失败时安全回退到普通关键词搜索；
- 暂不引入独立向量数据库，只有Meilisearch在获批金标和真实顾问试点中仍无法满足需求时才重新评估。

阶段4智能搜索意图化重构：

- 先把自然语言需求归一为四种受限意图：`semantic_search`、`catalog_browse`、`count`和`analysis_required`；模型只负责输出受Schema限制的计划，服务端根据明确的目录、计数、分析和语义线索进行第二次确定性校正；
- `catalog_browse`和`count`由服务端遍历Meilisearch分页并确定性去重、排序和计数，不允许DeepSeek删减符合条件的结果。目录默认每页20条、最大50条；一次运行最多冻结10000条结果，超过时安全失败并建议缩小条件；
- `semantic_search`最多召回60条候选，只允许DeepSeek在候选白名单中筛选和解释最多20条；结构校验、白名单校验或语义召回失败时，回退到关键词结果并明确标记限制；
- `analysis_required`不在搜索页基于有限候选计算比例、趋势、比较、相关性或原因，返回分流说明并引导进入分析工作区；
- 年份条件统一转换为左闭右开的日期范围。讲座按自身`date`筛选，案例按来源讲座派生的`source_date`筛选；案例索引字段变更必须通过临时索引构建和原子交换一次性发布；
- 每次智能搜索冻结意图、条件、限制、总数、分类型计数和有序结果快照。历史记录从冻结快照分页读取；阶段3旧记录仍按原单页结果兼容显示并标记为旧版，不伪造精确总数；
- 结果摘要由服务端根据意图、确定性计数和合法结果生成，不能把模型生成的未核验数量当作精确统计；各失败阶段只返回安全错误，不记录密钥、模型原始响应或知识正文；
- 结果卡继续支持单条加入工作区、批量勾选、全选当前页和统一加入工作区，服务端按`workspace_id + source_type + source_id`幂等去重并重新执行授权校验。

不建议在尚未建立检索评测集之前，以纯向量检索替代现有结构化搜索。

### 12.3 分析工作区、对话与报告

- 分析工作区默认私有，只能显式共享给指定内部账号；权限为`owner/editor/viewer`，管理员不自动越权读取；
- 工作区资料冻结到加入时的知识版本和内容哈希。出现新版本时提示主动更新，不静默替换，旧版本关系继续保留；
- 普通搜索和智能搜索支持勾选当前结果后批量加入工作区；工作区内也可按讲座／案例浏览当前正式知识目录、全选或勾选引入。相同类型和来源ID已存在时幂等跳过，不创建重复资料；
- 每个工作区最多保存500项冻结资料，以容纳当前完整讲座与案例目录。该收藏容量不扩大单次模型上下文：超出预算时仍只选取最多20项相关资料并按确定性规则压缩；
- 一个工作区可包含多个对话。模型输入只允许当前对话和工作区冻结资料；其他对话不得进入上下文；
- 超出上下文预算时按确定性规则压缩为相关资料、最近消息和版本化摘要，不能跨工作区补充资料；
- 分析报告先冻结资料快照和对话序列。数值与分类统计由服务端确定性计算，模型只生成结构化说明和合法引用；
- 固定模板输出交互式与静态单页HTML。交互仅包含缩放、章节聚焦、折叠、页内搜索和图例强调，不允许外部网络、动态代码或修改正式报告；
- 报告HTML按SHA-256写入知识域不可变对象存储，重生成形成新版本，不能覆盖旧产物。
...
- DOCX／PDF正式导出；
- 对外API和内部MCP；
- 在获批智能搜索金标和真实顾问试点基础上继续调优混合检索，并按实际需求评估知识图谱。
