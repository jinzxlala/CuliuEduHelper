...
describe("Auth.js options", () => {
  ...
  it("uses a system-specific cookie name and preserves the secure prefix rule", () => {
    const options = createAuthOptions(
      { authenticate },
      {
        CULIU_AUTH_COOKIE_NAME: "culiu-knowledge.session-token",
        NEXTAUTH_SECRET: "synthetic-secret-with-at-least-32-bytes",
        NEXTAUTH_URL: "https://knowledge.example.invalid",
      },
    );
    expect(options.cookies?.sessionToken?.name).toBe("__Secure-culiu-knowledge.session-token");
  });
