...
export function StudentImportForm(): JSX.Element {
  ...
  for (const candidate of batch?.candidates ?? []) {
    const phone = candidate.suggestions.find(
  ...
  const hasSharedParentPhone = [...phoneCounts.values()].some((count) => count > 1);
  ...
  async function decideCandidate(
    ...
    try {
      const fields = candidate.suggestions.map((suggestion) => {
        ...
        return {
          decision: decision !== "rejected" && accepted?.checked === true ? "accepted" : "rejected",
  ...
  return (
    <div className="import-workspace">
      ...
      {hasSharedParentPhone ? (
        <p className="warning-note">
          检测到候选学生共用家长电话。兄弟姐妹可能共用号码，请逐名核对；系统不会因此自动合并。
        </p>
      ) : null}
      {batch?.candidates.map((candidate) => (
        <section className="import-panel" key={candidate.id}>
          ...
          {candidate.possibleStudentId !== null ? (
            <p className="warning-note">
              检测到一个唯一的可能既有学生。请人工核对后选择关联，系统不会自动合并。
            </p>
          ) : null}
          ...
          {candidate.decision === "pending" ? (
            <div className="candidate-actions">
              ...
              {candidate.possibleStudentId !== null ? (
                <button
                  disabled={busy}
                  onClick={() => void decideCandidate(candidate, "link")}
                  type="button"
                >
                  关联已有学生
                </button>
