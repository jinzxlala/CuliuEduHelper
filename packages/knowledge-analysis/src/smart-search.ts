...
export const KNOWLEDGE_SMART_SEARCH_PROMPT_VERSION = "knowledge-smart-search.v1" as const;
export const KNOWLEDGE_SMART_SEARCH_SCHEMA_VERSION = "knowledge-smart-search-output.v1" as const;
export const KNOWLEDGE_SMART_SEARCH_RETRIEVAL_VERSION = "knowledge-hybrid.v1" as const;
...
export async function executeKnowledgeSmartSearch(
  ...
  try {
    ...
    for (let round = 1; round <= 2; round += 1) {
      ...
      const plan = SmartSearchQueryPlanSchema.parse(planned.json);
      ...
      await database
        .update(knowledgeSmartSearchRuns)
        .set({ progressStage: "retrieving", queryPlan: { rounds: plans, semanticRatio } })
        .where(eq(knowledgeSmartSearchRuns.id, run.id));
    ...
    const reranked = await provider.generateJson({
    ...
    const output = RerankOutputSchema.parse(reranked.json);
    ...
    const results = output.results.map((item) => {
      ...
      if (reference === undefined || seen.has(key))
        throw new Error("smart_search_result_outside_candidates");
      seen.add(key);
