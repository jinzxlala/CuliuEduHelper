...
const CONTEXT_FULL_SOURCE_CHARACTER_LIMIT = 120_000;
...
function compactSource(source: FrozenSource): Record<string, unknown> {
  const serialized = stableJson(source.content);
  return { reference: source.reference, summary: serialized.slice(0, 800), title: source.title };
}
...
async function buildSourceContext(
  ...
  if (allFull.length <= CONTEXT_FULL_SOURCE_CHARACTER_LIMIT)
    return {
      catalog: sources.map(compactSource),
  ...
  const hybridKeys = await expandedSourceKeys(search, question);
  const expanded = [...sources]
    .sort((left, right) => {
      ...
      const rightHybrid = hybridKeys.has(
        ...
        : 0;
      return (
        rightHybrid - leftHybrid || relevanceScore(right, question) - relevanceScore(left, question)
      );
    })
    .slice(0, MAX_EXPANDED_SOURCES)
    .map((source) => ({
      content: stableJson(source.content).slice(0, 5_000),
      reference: source.reference,
      title: source.title,
    }));
  return { catalog: sources.map(compactSource), expanded };
}

function assertCitations(citations: KnowledgeCitation[], sources: FrozenSource[]): void {
  const allowed = new Set(sources.map((source) => stableJson(source.reference)));
  for (const citation of citations) {
    KnowledgeCitationSchema.parse(citation);
    if (!allowed.has(stableJson(citation.source))) throw new Error("citation_outside_workspace");
  }
}

function assertSafeMarkdown(markdown: string): void {
  if (/<\/?[a-z][^>]*>/iu.test(markdown)) throw new Error("unsafe_markdown_html");
}
...
export async function executeKnowledgeAnalysisChat(
  ...
  try {
    ...
    if (throughSequence > (previousSummary?.throughSequence ?? 0)) {
      ...
      const generated = await provider.generateJson({
        ...
        userPrompt: stableJson({
          ...
          messages: summaryInput.map((message) => ({
            content: message.contentMarkdown,
            role: message.role,
            sequence: message.sequence,
          })),
    ...
    const sourceContext = await buildSourceContext(sources, lastUser.contentMarkdown, search);
    const history = messages.slice(recentStart).map((message) => ({
      content: message.contentMarkdown,
      role: message.role,
      sequence: message.sequence,
    }));
    ...
    usages.push(generated.usage);
    const output = KnowledgeAssistantMessageSchema.parse(generated.json);
    assertSafeMarkdown(output.answerMarkdown);
    assertCitations(output.citations, sources);
    ...
    await database
      ...
      .set({
        ...
        safeErrorCode: error instanceof z.ZodError ? "model_output_invalid" : "analysis_failed",
        safeErrorSummary: "本次分析未能安全完成，请重试。",
        status: "failed",
      })
      .where(eq(knowledgeAgentRuns.id, run.id));
    throw error;
