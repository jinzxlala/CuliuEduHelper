...
本仓库目前没有获批的真实课程目录，自动化测试只在随机临时数据库中使用明确标注的虚构课程，并在测试结束后删除。学生规划、替代方案、例外批准和 Markdown 导出已经具备技术工作流，但仍须使用获批课程目录和受控学生案例完成业务验收。
...
管理员可从 `/students/import` 批量上传 `.txt`、`.md`、`.docx` 或 `.csv`，由 Worker 调用服务端 DeepSeek 适配器提取基础信息；系统先保存导入批次和逐条预览，确认后才创建学生。学生详情页支持上传教学反馈 CSV 或家长会议 DOCX，模型只接收指定学生相关的最小化、脱敏文本，提取结果以逐条事实建议呈现，顾问可分别接受或拒绝。接受新事实会保留来源证据，并把相关画像、已接受课程推荐和规划标记为需要复查；重新生成画像必须由人工主动触发。

课程推荐要求存在已批准画像和已批准课程目录。系统先执行先修、重复修读和互斥等确定性规则，再让 DeepSeek 仅针对冻结后的可选课程模板与实际班级生成 1—3 个推荐和一个替代方案。推荐不会自动改写课程规划，顾问接受后可在规划页带入继续审核。

## 教师分配与班级课表优化

管理员入口为 `/scheduling`。该页面维护教师、固定地点、实际班级及每个班级的完整候选课表，并可提交后台求解任务。当前排课边界为：

- 教师在运行前尚未分配，由求解器同时选择教师和课表；
- 地点在运行前已经固定到班级，求解器不选择地点；
- 不执行学生自动分班，文本学生名单不参与冲突检测；
- 每个二进制变量对应“班级—候选教师—完整候选课表”；
- 教师资质、允许教师范围、教师与地点重叠、可用时间、每日／每周课时上限和人工锁定均作为硬约束。

求解结果先以草稿保存，包含未排班级及安全原因，必须由管理员显式批准后才能成为正式版本。HiGHS WASM 在 Worker 启动时执行自检，生产运行时不得静默降级为贪心排课。
