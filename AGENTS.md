...
- 当前已完成本地 Meilisearch 预备环境、第一项独立模块“工程脚手架与质量基线”，以及第二项独立模块“阶段 0 数据基础设施与边界”：PostgreSQL、Redis／BullMQ、Drizzle 迁移、本地不可变文件存储和脱敏 fixtures 已可用。
- 模型接口、业务页面、正式数据导入器和完整用户流程尚未实现；不要把阶段 0 基础设施误报为业务 MVP 已完成。
...
- `infra/setup-meilisearch.ps1`：幂等部署与索引初始化脚本，也是索引 settings 的当前来源文件。
...
索引的 searchable、filterable 和 sortable attributes 已在 `infra/setup-meilisearch.ps1` 中声明。修改字段或查询协议时，应同步更新：

1. `project_design.md` 中的索引设计；
2. 初始化脚本中的 settings；
3. 后续 TypeScript 搜索包中的类型与查询实现；
...
- `.github/workflows/ci.yml`：在 Node.js 22 下执行冻结安装、`pnpm check`、数据库迁移和 PostgreSQL／Redis 集成测试；
...
`pnpm check` 依次执行格式检查、潜在密钥扫描、Lint、严格类型检查、迁移一致性、17 个单元测试、Web／Worker 生产构建，以及首页和 `/api/health` 的真实 HTTP 冒烟测试。上述流程已在本机环境和一次性官方 Node.js 22 Docker 容器中通过。开发机当前 Node.js 主版本不是 22 时会出现 engine 警告，正式兼容性以 Node.js 22／CI 验收为准。
...
阶段 0 已完成。下一个独立模块进入 `dev_plan.md` 阶段 1“知识库导入与搜索”，合理顺序是：

1. 定义 TypeScript 搜索领域类型和三个索引的文档 Schema；
2. 明确 `data_origin/` 到 `lectures`、`cases`、`transcript_segments` 的证据映射；
3. 编写可重复、可审计、可重建的导入器；
4. 建立至少 50 条中文搜索金标查询；
