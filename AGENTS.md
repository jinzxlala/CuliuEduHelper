...
- 当前已完成本地 Meilisearch 预备环境、工程脚手架与质量基线、阶段 0 数据基础设施与边界、阶段 1 的搜索领域契约、48 场知识来源清单、正式 Worker 导入链路、版本化中文搜索评测器和内部知识搜索 Web 流程，以及阶段 2 的内部账号、Auth.js 会话、学生级授权基础、学生事实与不可变证据档案。
- 50 条中文金标查询已有 Code Agent 草案并通过技术评测，但仍待项目负责人逐条确认；逐字稿时间戳仍受隐私门禁保护。学生画像草稿技术流水线已经实现，人工复核/批准、课程匹配和规划报告尚未实现。不要把技术评测、页面可用、画像草稿或其他技术切片完成误报为阶段 1／阶段 2 业务门禁或业务 MVP 已完成。
...
2. 内部账号、会话、统一授权上下文、学生事实、不可变证据档案和 DeepSeek 画像草稿流水线已经完成；下一个阶段 2 模块应按 `dev_plan.md` 实现画像人工复核/批准/退回状态机和证据失效后的 `needs_review` 传播，仍只允许使用脱敏/虚构 fixture；
