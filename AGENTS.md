...
- 当前已完成本地 Meilisearch 预备环境，以及第一项独立开发模块“工程脚手架与质量基线”：Next.js Web、Node Worker、共享类型包、严格检查、测试、构建、运行时冒烟和 CI 已可用。
- PostgreSQL、Redis／BullMQ、本地文件存储适配器、模型接口、业务页面和正式数据导入器尚未实现；不要把工程骨架误报为业务 MVP 已完成。
...
- `.github/workflows/ci.yml`：在 Node.js 22 下执行冻结安装和 `pnpm check`；
...
`pnpm check` 依次执行格式检查、潜在密钥扫描、Lint、严格类型检查、5 个单元测试、Web／Worker 生产构建，以及首页和 `/api/health` 的真实 HTTP 冒烟测试。上述流程已在本机环境和一次性官方 Node.js 22 Docker 容器中通过。开发机当前 Node.js 主版本不是 22 时会出现 engine 警告，正式兼容性以 Node.js 22／CI 验收为准。

`infra/.env` 中的 `DEEPSEEK_API_KEY` 已做存在、非空、格式与最小长度检查，但本模块未调用 DeepSeek；不得在日志、文档、测试夹具或回复中输出密钥值。后续模型模块仍须通过服务端适配器读取。

## 8. 破坏性操作与工作区保护

以下命令会永久删除本地 Meilisearch 数据卷，只有在用户明确要求重置全部本地索引数据时才能执行：

```powershell
docker compose --env-file .\infra\.env -f .\infra\docker-compose.yml down -v
```

普通停止服务应使用不带 `-v` 的 `down`，以保留索引：
...
下一个独立模块仍属于 `dev_plan.md` 阶段 0，应先补齐 PostgreSQL、Redis／BullMQ、本地文件存储适配器、数据库迁移和脱敏 fixtures，再进入知识导入与搜索业务。其后合理顺序是：
