...
3. 阶段 3 的版本化课程目录和确定性规则基础已经完成；下一个代码模块实现基于已批准画像与已批准课程快照的人工课程规划、替代路线、规则覆盖记录和 Markdown 导出，继续完整参考 `$student-course-plan` 但不得引入动态 Skill Runtime；真实课程数据未到位前只能使用脱敏/虚构 fixture；
