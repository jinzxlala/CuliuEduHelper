...
- 普通搜索和智能搜索结果均可加入工作区。资料冻结到加入时的 PostgreSQL 知识版本和内容哈希；当前版本变化时只提示主动更新，更新会新增来源修订，不静默替换或永久删除旧来源；每个工作区最多 500 条资料、50 个对话，单次模型上下文仍只按确定性规则选取最多 20 条相关资料；
...
## 40. 搜索结果与分析工作区批量引入资料（2026-08-07）

- 普通搜索与智能搜索的讲座／案例结果新增“批量选择”模式。使用者可勾选当前结果、全选当前结果、选择一个可编辑工作区后统一加入；进入选择模式时原有单条加入控件会隐藏，退出后仍可按原流程逐条加入；
- 工作区新增 `/analysis/<workspace-id>/sources` 二级页面和“引入资料”入口。页面按“讲座／案例”两个标签展示当前已发布知识批次的完整资料目录，支持标题／摘要筛选、全选全部资料或当前筛选结果、清空选择、查看来源详情和批量加入；已在工作区中的资料会禁用并标记“已在工作区”；
- 去重由服务端保证：同一工作区中的 `source_type + source_id` 重复提交返回 `existing`，不会新增第二条记录。批量请求先一次性解析当前知识版本，再在工作区级 PostgreSQL advisory lock 事务中计算新增数量和写入，避免并发请求突破容量或产生部分状态；
- 为允许一次引入当前完整知识目录，工作区资料库容量从 100 调整为 500，数据库迁移为 `0026_expand_workspace_source_capacity.sql`。该调整只扩大人工资料库，不改变模型上下文：每次对话和报告仍按确定性相关性规则最多使用 20 条资料；
- 新增资料目录只读接口 `GET /api/analysis/workspaces/<id>/sources?type=lecture|case`，沿用工作区成员权限与 `private, no-store` 边界；批量写接口每次最多接收 500 个严格来源引用，浏览器仍不接触数据库、Meilisearch 或管理密钥；
- 选中／去重工具单元测试 2/2、知识 Web 单元测试 28/28、知识分析单元测试 12/12、真实 PostgreSQL 工作区集成测试 8/8、全仓串行集成 25/25 任务通过；知识 Web 生产构建、引入资料新路由和未登录页面／API 运行时冒烟通过；根 `pnpm check` 的格式、475 个仓库文本文件密钥扫描、Lint、严格类型、迁移一致性、全仓单测、双 Web 边界、17 包生产构建和双 Web 冒烟全部通过；
- 本轮功能模拟验证了“首次加入、重复提交不新增、目录回显已加入”和权限边界。既有 `pnpm test:functional:stage3` 在本轮新增功能完成后运行到旧搜索金标门禁时停止：本地正式语料已增长至 54 场讲座／219 个案例，而草案金标仍引用旧案例身份，普通金标为 48/50、智能金标为 0/30；不得把该语料版本错配误报为批量工作区失败，也不得降低阈值绕过。应另行重新绑定当前语料版本并由项目负责人审核金标；
- 本轮结论仅为“批量资料引入的本地技术功能测试通过”。仍需顾问在真实页面验证大量全选时的可读性、工作区容量策略和实际分析质量；当前开发机 Node.js 24 的 engine 警告与 BullMQ 可选 `@valkey/valkey-glide` 构建警告均为既有非阻断项，正式镜像继续使用 Node.js 22。

## 41. 人工审核发布超时与生产镜像 Logo 修复（2026-08-07）

- 生产故障并非提取稿内容错误：数据库中的多次发布尝试均在 `stage=search` 失败，Meilisearch 记录显示讲座文档任务约 15.8 秒完成、案例文档任务约 60.36 秒完成；Knowledge Web 过去没有把生产的 `MEILI_TASK_TIMEOUT_MS=600000` 传给 `KnowledgeIndexManager`，因此沿用 60 秒默认值并在案例任务即将成功时判定超时、清理临时索引。Nginx 通用 API 上游等待又固定为 65 秒，最终向浏览器返回 504；失败批次没有成为 current，原有正式索引未被替换。
- 公共搜索发布器的默认任务等待改为 600000 毫秒；Knowledge Web 新增严格环境变量解析并显式传入 500 毫秒轮询和 600000 毫秒等待；生产 Compose 同步向 Knowledge Web 注入这两个参数。Nginx 为 `PATCH /api/knowledge/imports` 的同步兼容发布接口单独设置 660 秒读写超时，使应用有机会返回明确的 Meilisearch 失败，而不是由网关提前截断。长期仍应把人工审核发布改为异步任务加状态轮询，本轮没有改变现有接口状态机。
- 新增确定性超时模拟：同一虚构 Meilisearch 文档任务被设为 60363 毫秒完成，显式旧 60000 毫秒配置必须失败，新的默认 600000 毫秒配置必须完成三索引原子交换。搜索包 33 项单元测试、Knowledge Web 31 项单元测试通过；真实 `pnpm test:integration` 最终 25/25 个任务包通过，其中搜索 10 项、知识发布 4 项和 Worker 3 项。第一次全量集成在高负载后使 Worker 故意失败重试用例超过固定 15/30 秒测试窗口；该包单独复跑 3/3 通过后，只把测试等待窗口提高为 30/45 秒，不修改生产三次重试和一秒退避，第二次全量集成通过。
...
## 44. 第四阶段智能搜索意图化重构（2026-08-07）

本轮以`docs/dev_plan_stage4.md`为实现边界，把原先只适合“找几条相关资料”的智能搜索拆成`semantic_search`、`catalog_browse`、`count`和`analysis_required`四种受限意图。模型负责理解自然语言和生成结构化计划；精确目录、计数、分页、年份范围、结果冻结、权限和去重均由服务端确定性代码负责。

### 意图、检索与日期语义

- 目录浏览和计数使用Meilisearch确定性分页遍历、跨计划去重和稳定排序，默认每页20条、最大50条；单次运行最多冻结10000条，超过上限安全失败。计数不把模型估算当作事实，`exact_total`必须等于讲座数与案例数之和；
- 语义搜索最多召回60条候选，只允许DeepSeek在候选白名单内筛选最多20条。模型规划和重排的结构错误各自最多提供一次受限修复；引用越界或重复只在尚未使用修复机会时纠正一次，仍不合法则安全失败；
- 需要比例、趋势、比较、相关性或原因判断的请求进入`analysis_required`，搜索页不根据有限候选直接给出结论，只显示资料范围建议和分析工作区入口；
- 案例索引新增`source_date`，由绑定的来源讲座当前发布版本确定性派生。年份筛选使用左闭右开的日期范围：讲座按`date`，案例按`source_date`；字段配置仍由`packages/search/index-definitions.json`统一管理，并通过临时索引与原子交换重建正式索引；
- 新版运行在PostgreSQL冻结意图、条件、限制、精确总数、分类型计数和有序结果详情；历史页面从冻结表分页读取。阶段3旧记录继续兼容为单页并标记旧版，不伪造精确总数。

### 页面与工作区兼容

- 智能搜索页显示意图、服务端总结、已应用条件、限制、精确总数或语义结果数；案例结果明确标注来源讲座日期。目录页在URL中保留`run`、`page`和`pageSize`，进入来源详情后返回仍可恢复同一次冻结查询；
- 原有单条加入工作区、批量选择、全选当前页和统一添加均保留。翻页时清空前一页浏览器选择，服务端继续按工作区与来源身份幂等去重，并重新检查工作区权限；
- `analysis_required`只提供进入分析工作区的分流，不把统计分析伪装成搜索结果。模型、Meilisearch或分页阶段失败时只保存安全阶段码和摘要，不保存模型原始响应、知识正文或服务密钥。

### 数据、任务与部署

- 数据库迁移`0027_yellow_gravity.sql`新增意图化运行元数据及不可变`knowledge_smart_search_result`冻结表；更新和删除冻结结果由PostgreSQL触发器拒绝；
- 智能搜索任务升级为`knowledge-smart-search.v3`、`knowledge-intent-search.v1`和`knowledge-smart-search-output.v2`，读取端继续接受阶段3的v2/v1旧任务与历史结果；
- 新增`pnpm test:deepseek:stage4`真实模型验收命令，只输出意图、计数、分页和Token统计，不输出模型原始响应或密钥；
- `infra/deploy/scripts/deploy-current-sha.sh`从当前检出的40位Git SHA设置`CULIU_IMAGE_TAG`与`CULIU_GIT_COMMIT_SHA`，只拉取应用镜像后重建生产服务，不删除卷或宿主机数据，也不要求手工修改环境文件中的旧标签。

### 测试、缺陷与修复记录

- 首次`pnpm check`在`appliedConditions`的两个多余字符串分支上被严格ESLint阻断；删除类型系统已证明不可能的`undefined`分支并正确保留`null`处理后，对应包Lint、类型检查和13项单元测试通过，未降低规则；
- 首次最终DeepSeek复跑虽然四种意图均正确，却因正式Meilisearch被测试流程留为空索引而全部返回0条；旧脚本只检查意图，错误地报告通过。已把非空目录、讲座/案例联合结果、语义非空和分类型边界写成硬断言，并从PostgreSQL当前批次重新原子构建54场讲座、219个案例和0条逐字稿索引；
- 索引恢复后真实DeepSeek五条验收依次通过：2025年讲座目录18条；2025年讲座18条加来源日期属于2025年的案例82条、合计100条和5页；AI与医疗语义搜索从15场讲座和13个案例候选中返回9条；2025年讲座计数18；案例比例请求进入`analysis_required`且返回0张搜索卡。该轮合计提示Token 18455、完成Token 1572、总Token 20027；
- 真实模型复跑还捕获一次`limitations`返回字符串而非数组的重排结构错误。重排现增加一次结构化受限修复并在同一修复预算内继续执行白名单校验；修复后知识分析Lint、类型和10项真实PostgreSQL集成测试通过，原候选越界纠正用例保持通过；
- 最终整仓复跑暴露了既有Operations Web冒烟隔离缺陷：临时Worker使用临时空数据库却仍执行启动时知识索引对账，既会等待共享Meilisearch嵌入任务而超过15秒就绪窗口，也会把本地正式索引重建为空。Worker新增仅允许`NODE_ENV=test`使用的`KNOWLEDGE_STARTUP_RECONCILE_ENABLED=false`开关，Operations Web冒烟显式关闭该步骤；生产和开发环境禁止关闭，继续在启动时对账正式索引。Worker 11项单元测试、Lint、类型、构建和完整Operations Web HTTP冒烟通过，正式索引随后从PostgreSQL当前批次恢复为54/219/0；
- 根`pnpm check`完整通过：482个仓库文本文件密钥扫描、Lint、严格类型、迁移一致性、全仓单测、双Web边界、Nginx布局、17包生产构建和双Web HTTP冒烟均成功。全量`pnpm test:integration`为25/25个任务包通过，其中搜索11项、知识分析10项、知识导入6项和Worker 3项；
- 当前只能声明“第四阶段本地技术功能与真实模型查询验收通过”。30条智能搜索金标仍须重新绑定当前54/219语料版本并由项目负责人审核，之后还需真实顾问试点；不得把五条验收查询或镜像推送误报为业务验收完成。开发机Node.js 24 engine警告和BullMQ可选`@valkey/valkey-glide`构建警告仍为既有非阻断项，生产镜像继续使用Node.js 22。

## 45. 大型工作区分析报告分批与输出预算（2026-08-07）

- 工作区含约200条资料时，旧报告任务会把完整来源一次性送入最终模型调用，可能在调用前因上下文过大立即失败。报告生成现按输入字符预算分批归纳来源，再在必要时分层合并证据摘要，最后只把确定性图表、对话要求和可追溯摘要交给撰稿模型；每次调用继续限制输入规模并逐批校验证据引用，不能借分批绕过冻结来源白名单；
- 首轮分批修复后，生产真实任务先后在8192和16384输出Token处被DeepSeek截断。根因不是输入仍超限，而是旧输出契约最多允许20章、每章20段和100条引用，模型面对大量资料时倾向逐条展开。报告提示词升级为`knowledge-analysis-report.v4`，广泛资料默认目标为8—10章、每章2—4段、每章3—5条代表性引用，执行摘要约800—1400字；完整资料仍由工作区与冻结快照保存，不要求正文逐条枚举；
- 报告任务不再与画像、逐字稿提取等流程共用输出参数。新增`DEEPSEEK_REPORT_MAX_TOKENS`和`DEEPSEEK_REPORT_TIMEOUT_MS`，本地与生产默认分别为24576和180000毫秒；普通模型任务继续使用`DEEPSEEK_PROFILE_MAX_TOKENS`和`DEEPSEEK_PROFILE_TIMEOUT_MS`。生产Compose只向Worker注入报告专用参数，浏览器端不得读取；
- 最终叙事契约限制为最多10章、每章最多4段和5条代表性引用；批次摘要最多12条代表性引用、2500字摘要和6条边界说明。首次返回`finish_reason=length`或完整JSON超出新叙事Schema时，系统在同一任务内只自动重试一次，改用6—8章、每章2—3段的紧凑要求；第二次仍失败则保留安全失败，不无限重提；
- 200条大型合成来源测试继续确认所有模型输入不超过上限，新增“首次输出截断后紧凑重试”和“首次结构超出章节预算后重试”用例。`@culiu/ai`6项、`@culiu/knowledge-analysis`18项、`@culiu/tasks`14项定向单元测试通过；根`pnpm check`完整通过，全量`pnpm test:integration`为25/25个任务包通过。测试使用确定性模型适配器，未调用真实DeepSeek；生产部署后仍须由项目负责人用同一大型工作区重新生成，确认实际篇幅、引用质量、耗时和成本。
