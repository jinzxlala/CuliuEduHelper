...
- 历史逐字稿兼容流程的模型、提示词和 Schema 身份升级为 `knowledge-transcript-extraction.v5`／`knowledge-analysis-markdown.v5`。DeepSeek 必须综合原文件名线索与逐字稿正文返回讲座日期、主题、置信度和简短依据；只有月日而正文不能确定年份时日期必须为 `null`，不得用上传日期、当前日期或常识补齐。原文件继续按原名不可变保存，正式 `source_key` 和 `lecture_id` 只在人工确认完整日期与主题后生成；页面允许修订两项元数据，缺少任一项时禁止发布；
