...
- 当前已完成本地 Meilisearch 预备环境、工程脚手架与质量基线、阶段 0 数据基础设施与边界、阶段 1 的搜索领域契约、48 场知识来源清单、正式 Worker 导入链路、版本化中文搜索评测器和内部知识搜索 Web 流程，以及阶段 2 的内部账号、Auth.js 会话、学生级授权基础、学生事实与不可变证据档案、画像草稿和人工审核状态机。
- 50 条中文金标查询已有 Code Agent 草案并通过技术评测，但仍待项目负责人逐条确认；逐字稿时间戳仍受隐私门禁保护。学生画像技术链路已具备修改、提交、退回、批准、归档和证据失效复查能力，但 20—30 个真实受控案例尚未进入业务验收；课程匹配和规划报告尚未实现。不要把技术评测、页面可用、画像技术链路或其他技术切片完成误报为阶段 1／阶段 2 业务门禁或业务 MVP 已完成。
...
3. 下一个代码模块进入阶段 3：先实现自有课程目录和确定性课程规则的领域契约/数据库基础，只能使用脱敏/虚构 fixture；涉及学生后续课程规划时必须完整阅读并参考 `$student-course-plan`，但不得引入动态 Skill Runtime；
