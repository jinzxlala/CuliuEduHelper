...
- 当前已完成本地 Meilisearch 预备环境、工程脚手架与质量基线、阶段 0 数据基础设施与边界、阶段 1 的搜索领域契约、48 场知识来源清单、正式 Worker 导入链路、版本化中文搜索评测器和内部知识搜索 Web 流程，阶段 2 的内部账号、Auth.js 会话、学生级授权基础、学生事实与不可变证据档案、画像草稿和人工审核状态机，以及阶段 3 的版本化课程目录和确定性课程规则基础。
- 50 条中文金标查询已有 Code Agent 草案并通过技术评测，但仍待项目负责人逐条确认；逐字稿时间戳仍受隐私门禁保护。学生画像技术链路已具备修改、提交、退回、批准、归档和证据失效复查能力，但 20—30 个真实受控案例尚未进入业务验收；真实课程目录尚未提供，学生课程规划、替代路线、例外批准和 Markdown 报告尚未实现。不要把技术评测、页面可用、画像或课程规则技术切片完成误报为业务门禁或业务 MVP 已完成。
...
- 课程规划包现有 19 项单元测试和 6 项 PostgreSQL 集成测试；全仓 `pnpm check` 通过，共 135 项单元测试、生产构建和真实 Web 冒烟通过；`pnpm test:integration` 共 59 项全部通过。官方 Node.js 22.23.2 Linux 一次性容器也通过冻结安装、Lint、严格类型、迁移一致性、135 项单元测试和全部生产构建；
...
- 当前尚未把规划工作流接入 Web API 和内部操作页面，也没有获批的真实课程目录或真实学生案例，因此不能宣称阶段 3 或 MVP 业务验收完成。下一独立模块应完成规划 API、顾问页面和端到端授权/导出验收。
...
3. 阶段 3 的版本化课程目录、确定性规则和人工规划核心工作流已经完成；下一个代码模块把规划创建、审核、规则覆盖和 Markdown 导出接入受保护的 Web API 与内部顾问页面，并补齐端到端授权和失效展示；真实课程数据未到位前只能使用脱敏/虚构 fixture；
4. 真实学生案例、真实课程目录和授权规则未到位前，不导入真实资料、不宣称学生画像或课程规划业务验收完成；
5. 学生事实、证据和知识库案例继续分域；讲座案例不能被复制为学生事实，模型参数不能扩大服务端授权范围；
6. 继续记录真实搜索使用中的未命中、误命中和过滤错误，只依据确认后的评测调整字段权重或查询协议；
7. 只有确认后的评测证明关键词检索不足时，再讨论同义词、向量或混合检索。
